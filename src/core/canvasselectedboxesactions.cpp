...
bool isInsideRemovedSubtree(BoundingBox* const candidate,
    ...
    return candidate && removedRoot &&
           (candidate == removedRoot || candidate->isAncestor(removedRoot));
}

QString makeUniquePrecompName(Document& document,
                              const QString& baseName) {
    QSet<QString> usedNames;
    for (const auto& scene : document.fScenes) {
        if (scene) {
            usedNames.insert(scene->prp_getName().trimmed());
        }
    }

    QString candidate = baseName.trimmed();
    if (candidate.isEmpty()) {
        candidate = QObject::tr("Composition");
    }
    ...
    int index = 2;
    while (true) {
        const QString numbered = QStringLiteral("%1 %2").arg(candidate).arg(index);
...
BoundingBox *Canvas::precomposeSelectedBoxes()
{
    ...
    ContainerBox *sourceParent = nullptr;
    for (auto *box : selected) {
        if (!box) {
            continue;
        }
        sourceParent = box->getParentGroup();
        if (sourceParent) {
            break;
        }
    }
    ...
    QList<BoundingBox*> boxesToMove;
    for (auto *box : selected) {
        if (box && box->getParentGroup() == sourceParent) {
    ...
    const auto &siblings = sourceParent->getContainedBoxes();
    std::sort(boxesToMove.begin(), boxesToMove.end(),
              [&siblings](BoundingBox *lhs, BoundingBox *rhs) {
        return siblings.indexOf(lhs) < siblings.indexOf(rhs);
    });

    int insertIndex = siblings.count();
    for (auto *box : boxesToMove) {
        insertIndex = qMin(insertIndex, siblings.indexOf(box));
    }
    if (insertIndex < 0) {
        insertIndex = siblings.count();
    }

    const QString baseName = boxesToMove.count() == 1
            ? boxesToMove.first()->prp_getName()
            : QObject::tr("Pre-comp");

    auto *newScene = mDocument.createNewScene(false);
    if (!newScene) {
        return nullptr;
    }
    newScene->prp_setName(makeUniquePrecompName(mDocument, baseName));
    ...
    if (auto *bg = newScene->getBgColorAnimator()) {
        bg->setColor(getBgColorAnimator()->getColor(anim_getCurrentAbsFrame()));
    }
    emit mDocument.sceneCreated(newScene);

    auto link = newScene->createLink(false);
    link->prp_setName(newScene->prp_getName());
    sourceParent->insertContained(insertIndex, link);

    for (int i = boxesToMove.count() - 1; i >= 0; --i) {
        auto *box = boxesToMove.at(i);
        const auto boxRef = box->ref<BoundingBox>();
        boxRef->removeFromParent_k();
        newScene->addContained(boxRef);
    }
...
QList<BoundingBox*> Canvas::selectedBoxesList() const
{
