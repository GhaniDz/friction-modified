                                      ...
                                      const int frame)
{
    ...
    if (mimeData->hasFormat(kSceneMimeType)) {
        QByteArray payload = mimeData->data(kSceneMimeType);
        QDataStream stream(&payload, QIODevice::ReadOnly);
        quint64 scenePtrValue = 0;
        stream >> scenePtrValue;
        auto *scene = reinterpret_cast<Canvas*>(static_cast<quintptr>(scenePtrValue));
        if (scene && scene != mActiveScene) {
            event->acceptProposedAction();
            auto link = scene->createLink(false);
            link->prp_setName(scene->prp_getName());
            ...
            if (const auto importedBox = enve_cast<BoundingBox*>(link)) {
                ...
                mActiveScene->clearBoxesSelection();
                mActiveScene->addBoxToSelection(importedBox);
            }
            afterAction();
            return link.get();
        }
                                 ...
                                 const int frame)
{
    ...
    Canvas * const previousActiveScene = *mDocument.fActiveScene;
            ...
            try {
                if (isImageExt(extension)) {
                    ...
                    if (previousActiveScene) {
                        mDocument.setActiveScene(previousActiveScene);
                    }
                }
            } catch(const std::exception& e) {
                gPrintExceptionCritical(e);
...
void Actions::setPuppetPinMode() {
    mDocument.setCanvasMode(CanvasMode::puppetPinCreate);
}
