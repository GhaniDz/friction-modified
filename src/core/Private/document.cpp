...
QString makeUniqueSceneName(const QList<qsptr<Canvas>> &scenes,
                            const QString &baseName)
{
    ...
    int index = 2;
    while (true) {
        const QString candidate = QStringLiteral("%1 %2").arg(baseName).arg(index);
...
Canvas *Document::createNewScene(const bool emitCreated)
{
    ...
    newScene->prp_setName(makeUniqueSceneName(fScenes, QObject::tr("Composition")));
...
bool Document::removeScene(const int id)
{
    ...
    if (fActiveScene == scene.get()) {
        Canvas *nextScene = nullptr;
        if (!fScenes.isEmpty()) {
            const int nextIndex = qBound(0, id, fScenes.count() - 1);
            nextScene = fScenes.at(nextIndex).get();
        }
        setActiveScene(nextScene);
    }
    removeVisibleScene(scene.get());
