...
void BoundingBox::readBoundingBox(eReadStream& src) {
    ...
    if(src.evFileVersion() >= EvFormat::timelineColor) {
        src >> mTimelineColor;
    } else {
        mTimelineColor = QColor();
    }
...
bool BoundingBox::hasRasterEffects() const {
    return mRasterEffectsAnimators->ca_hasChildren();
}

QList<stdsptr<RasterEffectCaller>> BoundingBox::rasterEffectCallers(
        const qreal relFrame,
        BoxRenderData * const data,
        const qreal influence) const {
    QList<stdsptr<RasterEffectCaller>> result;
    if(!mRasterEffectsAnimators->SWT_isEnabled()) return result;
    if(isZero4Dec(influence)) return result;
    const auto resolution = data ? data->fResolution : 1.;
    ...
    for(int i = 0; i < totalEffects; ++i) {
        ...
        const auto caller = effect->getEffectCaller(relFrame, resolution, influence, data);
        if(caller) result << caller;
    }
    return result;
}
...
ContainerBox *BoundingBox::getFirstParentLayerOrSelf() const {
    if(isLayer()) {
        return const_cast<ContainerBox*>(enve_cast<const ContainerBox*>(this));
    }
    return getFirstParentLayer();
}
...
QColor BoundingBox::defaultTimelineColor() const
{
    switch (mType) {
    case eBoxType::text:
    case eBoxType::nullObject:
        return QColor(122, 54, 54);
    ...
    case eBoxType::layer:
    case eBoxType::group:
    case eBoxType::canvas:
        return QColor(88, 104, 152);
    default:
        return QColor(96, 116, 168);
    }
}

QColor BoundingBox::effectiveTimelineColor() const
{
    return mTimelineColor.isValid() ? mTimelineColor : defaultTimelineColor();
}

void BoundingBox::setTimelineColor(const QColor &color)
{
    if (!color.isValid() || mTimelineColor == color) { return; }
    {
        prp_pushUndoRedoName(tr("Set Timeline Color"));
        UndoRedo ur;
        const auto oldValue = mTimelineColor;
        const auto newValue = color;
        ur.fUndo = [this, oldValue]() {
            if (oldValue.isValid()) setTimelineColor(oldValue);
            else clearTimelineColor();
        };
        ur.fRedo = [this, newValue]() { setTimelineColor(newValue); };
        prp_addUndoRedo(ur);
    }
    mTimelineColor = color;
    emit timelineColorChanged(mTimelineColor);
}

void BoundingBox::clearTimelineColor()
{
    if (!mTimelineColor.isValid()) { return; }
    {
        prp_pushUndoRedoName(tr("Reset Timeline Color"));
        UndoRedo ur;
        const auto oldValue = mTimelineColor;
        ur.fUndo = [this, oldValue]() { setTimelineColor(oldValue); };
        ur.fRedo = [this]() { clearTimelineColor(); };
        prp_addUndoRedo(ur);
    }
    mTimelineColor = QColor();
    emit timelineColorChanged(mTimelineColor);
...
void BoundingBox::setParentTransformKeepTransform(BasicTransformAnimator *parent) {
    if(parent == mParentTransform) return;
...
void BoundingBox::clearParentKeepTransform() {
    const auto parent = getParentGroup();
    if(parent) setParentTransformKeepTransform(parent->getTransformAnimator());
    else setParentTransformKeepTransform(nullptr);
}
...
void BoundingBox::rebuildCanvasProps() {
    ...
    const auto appendCanvasProp = [this, &uniqueProps](Property* const prop) {
        if(!prop || !prop->prp_drawsOnCanvas()) {
            return;
        }
        if(uniqueProps.contains(prop)) {
            return;
        }
        uniqueProps.insert(prop);
...
void BoundingBox::ensureBlendEffectsVisible()
{
    if (!mBlendEffectCollection->SWT_isVisible()) {
        setBlendEffectsVisible(true);
    }
}
