                      ...
                      const bool mouseGrabbing)
{
    ...
    if (!enve_cast<Canvas*>(mCurrentContainer) && mSelectedBoxes.isEmpty()) {
    ...
    if (mCurrentMode == CanvasMode::boxTransform ||
       mCurrentMode == CanvasMode::pointTransform ||
       mCurrentMode == CanvasMode::puppetPinCreate) {
...
void Canvas::deleteAction()
{
    ...
    case CanvasMode::puppetPinCreate:
...
void Canvas::splitAction()
{
    ...
    for (const auto &selected : mSelectedBoxes.getList()) {
...
bool Canvas::startScalingAction(const eKeyEvent &e)
{
    if (mCurrentMode != CanvasMode::boxTransform &&
        mCurrentMode != CanvasMode::pointTransform &&
        mCurrentMode != CanvasMode::puppetPinCreate) { return false; }
