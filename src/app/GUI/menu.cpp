...
void MainWindow::setupMenuBar()
{
    ...
    mViewTimelineAct = mViewMenu->addAction(tr("View Layers"));
    mViewTimelineAct->setCheckable(true);
    mViewTimelineAct->setChecked(true);
    mViewTimelineAct->setShortcut(QKeySequence(tr("Ctrl+Alt+L")));
    ...
    mViewFillStrokeAct = mViewMenu->addAction(tr("View Effect Controls"));
    ...
    mPanelsMenu = mMenuBar->addMenu(tr("Panels", "MenuBar"));

    mPanelCompositionAct = mPanelsMenu->addAction(tr("Composition Viewer"));
    mPanelCompositionAct->setCheckable(true);
    connect(mPanelCompositionAct, &QAction::triggered,
            this, [this](bool triggered) {
                mUI->setDockVisible(tr("Composition"), triggered);
                if (triggered && mCenterTabs) {
                    mCenterTabs->setCurrentIndex(0);
                }
                syncPanelsMenuState();
            });

    mPanelProjectAct = mPanelsMenu->addAction(tr("Project"));
    mPanelProjectAct->setCheckable(true);
    connect(mPanelProjectAct, &QAction::triggered,
            this, [this](bool triggered) {
                mUI->setDockVisible(tr("Project"), triggered);
                syncPanelsMenuState();
            });

    mPanelEffectControlsAct = mPanelsMenu->addAction(tr("Effect Controls"));
    mPanelEffectControlsAct->setCheckable(true);
    ...
    mPanelLayersAct = mPanelsMenu->addAction(tr("Timeline / Layers"));
    mPanelLayersAct->setCheckable(true);
    ...
    mPanelsMenu->addAction(tr("Render Queue"),
                           this, [this]() {
    ...
    mPanelsMenu->addAction(tr("Float/Dock Composition Viewer"),
                           this, [this]() {
                               mUI->setDockFloating(tr("Composition"),
                                                    !mUI->isDockFloating(tr("Composition")));
                           });
    mPanelsMenu->addAction(tr("Float/Dock Project"),
                           this, [this]() {
                               mUI->setDockFloating(tr("Project"),
                                                    !mUI->isDockFloating(tr("Project")));
                           });
    mPanelsMenu->addAction(tr("Float/Dock Effect Controls"),
                           this, [this]() {
                               mUI->setDockFloating(tr("Effect Controls"),
                                                    !mUI->isDockFloating(tr("Effect Controls")));
                           });
    mPanelsMenu->addAction(tr("Float/Dock Timeline"),
                           this, [this]() {
    ...
    mPanelsMenu->addAction(tr("Save Layout"),
                           this, [this]() {
                               if (mUI) { mUI->writeSettings(); }
                               AppSupport::setSettings("ui", "state", saveState());
                               AppSupport::setSettings("ui", "geometry", saveGeometry());
                               statusBar()->showMessage(tr("Panel layout saved"), 2500);
                           });
    mPanelsMenu->addAction(tr("Reset to AE Layout"),
                           this, [this]() {
                               if (!mUI) { return; }
                               mUI->applyAeDefaultWorkspace();
                               mUI->setDockVisible(tr("Composition"), true);
...
{
    mSceneMenu = mMenuBar->addMenu(tr("Composition", "MenuBar"));

    const auto newSceneAct = mSceneMenu->addAction(QIcon::fromTheme("file_new"),
                                                   tr("New Composition", "MenuBar_Scene"),
                                                   this, [this]() {
    ...
    const auto deleteSceneAct = mSceneMenu->addAction(QIcon::fromTheme("cancel"),
                                                      tr("Delete Composition", "MenuBar_Scene"));
    ...
    const auto scenePropAct = mSceneMenu->addAction(QIcon::fromTheme("sequence"),
                                                    tr("Composition Settings", "MenuBar_Scene"));
